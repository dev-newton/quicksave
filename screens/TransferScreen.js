...
export default function TransferScreen(props) {
  ...
  const getNameInquiry = async (text) => {
    ...
    try {
      const response = await axios({
        ...
        url: `${BASE_URL}/api/heritagebank/identity/NameInquiry?AccountNumber=${text}`,
      });
      setLoadingAcc(false);
  ...
  const makeTransfer = async () => {
    ...
    try {
      ...
      Alert.alert('Transfer Successful', response.data.responseMessage, [
        {
          text: 'Cancel',
    ...
    <View style={styles.main}>
      <ScrollView>
        ...
        <View style={styles.amountWrapper}>
          <Text style={styles.amountText}>₦350,000</Text>
          <View style={styles.amountLine} />
          <Text style={styles.amountText1}>Total{'\n'} Balance</Text>
        </View>
        <View>
          <View style={styles.formWrapper}>
            <Text style={styles.pickerHeaderText}>Amount</Text>
            <TextInput
            ...
            <TouchableOpacity style={styles.button} onPress={makeTransfer}>
              {loadingTranx ? (
